...
def play_music(music_file):
    ...
    try:
        ...
        print(f"Couldn't play {music_file}! ({pygame.get_error()})")
        return
...
try:
    ...
    for song in matches:
        ...
        if do_ffmpeg_convert:
            os.system(
                f'ffmpeg -i {new_file} -y -f mp3 -ab {str(mp3_bitrate)}k -ac {str(channels)} -ar {str(sample_rate)} -vn {file_name}.mp3'
            )
